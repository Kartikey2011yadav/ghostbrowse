...
abstract class PigeonApiWebStorage(open val pigeonRegistrar: AndroidWebkitLibraryPigeonProxyApiRegistrar) {
   ...
   */
  abstract fun setTemporaryDirectory(pigeon_instance: android.webkit.WebStorage, tempDir: String)
  ...
  companion object {
    ...
    fun setUpMessageHandlers(binaryMessenger: BinaryMessenger, api: PigeonApiWebStorage?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.webview_flutter_android.WebStorage.setTemporaryDirectory", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pigeon_instanceArg = args[0] as android.webkit.WebStorage
            val tempDirArg = args[1] as String
            val wrapped: List<Any?> = try {
              api.setTemporaryDirectory(pigeon_instanceArg, tempDirArg)
              listOf(null)
            } catch (exception: Throwable) {
              wrapError(exception)
            }
